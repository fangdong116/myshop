...
class UserModel extends Model
{
    ...
    public static function createConsultation($params)
    {
        ...
        return Flight::db()->insert('ims_cs_consultation', $data);
    }
    ...
    public static function createMessage($params)
    {
        ...
        return Flight::db()->insert('ims_cs_message', $data);
    }
    ...
    public static function getMessageList($params)
    {
        $sql = "select
				...
				m.msg_type,
				m.created_time";
        $sql_body = " from ims_cs_consultation c
					inner JOIN ims_cs_message m on c.consultation_id = m.consultation_id
					where 1 ";
    ...
    public static function evaluateConsultation($consultation_id, $evaluation)
    {
        ...
        $sql = "update ims_cs_consultation
    ...
    public static function editDiagnosis($params)
    {
        ...
        return Flight::db()->update('ims_cs_diagnosis', $data, "  diagnosis_id = {$params['diagnosis_id']}");
    }

    public static function getDiagnosis($diagnosis_id)
    {
        $sql = "select * from ims_cs_diagnosis where diagnosis_id = {$diagnosis_id} ";
    ...
    public static function  getDiagnosisList($params)
    {
        ...
        $sql_body = " from ims_cs_diagnosis d where 1 ";
...
//			$diagnosis['meal_habit_types'] = self::getFormatTypeList($diagnosis['meal_habit_type_ids'], 'meal_habit_type');
    ...
    public static function insertOrUpdateCalender($year, $month, $day, $event_type, $dateline, $user_id, $user_name)
    {
        $sql = "select
                    *
                from
                ...
                and status = 'OK' ";
        $last_set_calendar = Flight::db()->getRow($sql);
        if (!isset($last_set_calendar)) {
            $data = array(
                'user_id' => $user_id,
                'user_name' => $user_name,
                'year' => $year,
            ...
            Flight::db() -> insert('ims_cs_calendar', $data);
        }
