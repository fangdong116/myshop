...
class cls_mms_tool
{
    ...
    private static function getApiUrl($key) {
        ...
        if (!isset($config['mms_config'][$key])) {
            return array('error_code' => '11111' , 'error_info' => '非法的mmsApi路径');
        }
        $url = $config['mms_config'][$key];
        return $url;
    }
    ...
    public static function post_data($url, $data) {
        try {
            ...
            \Logger::getLogger("Api")->debug("Exception mms:POST, url={$url}, exception_msg={$e->getMessage()}");
    ...
    public static function get_data($url) {
        try {
            ...
            \Logger::getLogger("Api")->debug("Exception mms:GET,url={$url}, exception_msg={$e->getMessage()}");
    ...
    private static function parseResponse($response, $response_key=null) {
        if (! isset($response) || empty($response) || !is_array($response) || ! isset($response['result']) || $response['result'] != "ok") {
            if (isset($response['error_message'])) {
                return array('error_code' => '11111' , 'error_info' => "mmsapi错误:{$response['error_code']},{$response['error_message']}", 'error_result' => json_encode($response));
            } else {
                return array('error_code' => '11111' , 'error_info' => 'mmsapi错误:请求未完成或返回格式错误', 'error_result' => json_encode($response));
            }
        }
        ...
        if (!isset($response[$response_key])) {
            return array('error_code' => '11111' , 'error_info' => 'mmsapi错误:返回格式错误','error_result' => json_encode($response));
        }
